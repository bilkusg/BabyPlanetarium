...
import {Effect,Scene,ShaderMaterial,Engine,Mesh,VertexData,VertexBuffer} from "@babylonjs/core"

Effect.ShadersStore["starVertexShader"] = `
...
`;
Effect.ShadersStore["starFragmentShader"] = `
precision highp float;
varying vec2 vUV;
...
`;
const createStarShaderMaterial = function (name:string, scene:Scene) {
    var starShaderMaterial = new ShaderMaterial(name, scene, { vertex: "star", fragment: "star", }, {
    ...
    starShaderMaterial.alphaMode = Engine.ALPHA_MAXIMIZED;
...
const createStars = function (scene:Scene, starDistance:number) {
    var positions = [];
    var colors = [];
    var starMagnitudes = [];
    ...
    var starMesh = new Mesh("stars", scene);
    ...
    //starMesh.setVerticesData(VertexBuffer.PositionKind,positions,true);
    starMesh.setVerticesData("starColor", colors, true, 4);
    starMesh.setVerticesData("starMagnitude", starMagnitudes, true, 1);
    //console.log("Star mesh = ",starMesh);
    //console.log("Positions = ",positions);
    starMesh.isPickable = true;
    return starMesh;
};
const createPlanets = function (scene:Scene,solarSystemResult:Astro.SolarSystemResult) {
    var planetMesh = new Mesh("planets", scene);
